...
class ColumnOrganizedStachExtension:
    ...
    def convert_to_table_format(package):
        ...
        for primary_table_id in package.primary_table_ids:
            tables.append(ColumnOrganizedStachExtension.__generate_table(package, primary_table_id))
        return tables
        ...
        if isinstance(package_response, Package):
            ...
            for series_definition_column in header_table.definition.columns:
                ...
                for primary_table_definition in primary_table.definition.columns:
                    ...
                    indexOfHeaderColumnId = [index for index in range(len(header_table.data.rows)) if header_table.data.rows[index].id == headerColumnId][0]
                    val = str(_SeriesDataHelper.get_value_helper(header_table.data.columns[series_definition_column.id], series_definition_column.type, indexOfHeaderColumnId, series_definition_column.format.null_format))
